...
State the paper's contribution factually; describe, do not advertise. Use these templates as a checklist for what each section should cover. The title, abstract, the introduction's second-to-last paragraph, the captions, the equations, and the conclusion together run under two pages but carry the paper — they are the frame that sets its level and the first place a reviewer judges the author's competence, so hold them to the highest standard.

- **Abstract:** background, gap, method, key quantified result, significance, scope or limits. Keep results specific but compact. The abstract is an expansion of the title; whenever the title is settled or changed, revise the abstract to match. Anticipate and answer the reader's doubt about why the research object and its properties are worth studying — state the motivation for the object briefly here, then argue it in full in the introduction.
- **Introduction:** field background and trends, existing work, the unsolved gap, this paper's contribution, then a one-paragraph roadmap of the rest of the paper. The roadmap conventionally opens with `The rest of the paper is organized as follows.` and ends with `The last section concludes the paper.`; keep this closing paragraph and follow the fixed phrasing. Write it as continuous prose of roughly two to three pages: no subsection headings, and no figure or table — a display fragments the narrative before the reader has the full framing, and its information density is too low to justify the space. Keep the framing broad, especially in the first paragraph, and introduce no table before the reader has a complete mental framework to place it in.
...
- **Title:** concise; centered on the research object and what the work does. Tailor it to the target journal's scope and topical preferences as well as to the work itself, so the manuscript clears the editor's pre-screen. Test a candidate title and its keywords by searching a database such as Web of Science for strongly related papers: the number and relevance of the hits are an objective gauge of whether the keywords are effective and well chosen.
...
- A figure or table is subordinate to the prose; it supplies verifying evidence or a sample case, and does not carry the argument itself. Do not open an analysis section with a display: lead with the general qualitative and theoretical account, then bring in the figure or table to confirm or illustrate it. Write "the periodic characteristics follow from …, as Fig. 9 confirms," not "Fig. 9 presents the periodic characteristics of the map" as the reasoning.

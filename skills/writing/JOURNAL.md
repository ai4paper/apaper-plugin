...
- **Introduction:** field background and trends, existing work, the unsolved gap, this paper's contribution, then a one-paragraph roadmap of the rest of the paper. The roadmap conventionally opens with `The rest of the paper is organized as follows.` and ends with `The last section concludes the paper.`; keep this closing paragraph and follow the fixed phrasing.
- **Contributions (when listed):** name each contribution as a noun or gerund phrase describing the contribution itself, not a sentence about author activity. Doing work is not a contribution — "Extensive experiments are conducted on two datasets" states an activity; the contribution is what those experiments establish, or a genuinely novel method or experimental design, not the act of running them. Reserve the list for methods, results, and insights.
...
- Keep captions self-contained: a reader should grasp the figure or table from its caption alone. (For caption formatting, see [LATEX.md](LATEX.md).)
